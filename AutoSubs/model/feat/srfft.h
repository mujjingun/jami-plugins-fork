...
private:
    void ComputeTables();
    void ComputeRecursive(Real* xr, Real* xi, int logn) const;
    void BitReversePermute(Real* x, int logn) const;
...
public:
    ...
    SplitRadixRealFft(const SplitRadixRealFft<Real>& other)
        : SplitRadixComplexFft<Real>(other)
        , N_(other.N_)
    {
    }
