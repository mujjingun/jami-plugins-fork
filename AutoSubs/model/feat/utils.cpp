...
//! ComplexMul implements, the complex multiplication b *= a.
template <typename Real>
static void ComplexMul(const Real& a_re, const Real& a_im,
    Real* b_re, Real* b_im)
{
...
template <typename Real>
static void ComplexAddProduct(const Real& a_re, const Real& a_im,
...
template <typename Real>
static void ComplexImExp(Real x, Real* a_re, Real* a_im)
{
    *a_re = std::cos(x);
    *a_im = std::sin(x);
}
    ...
    std::vector<Real>* tmp_vec)
{
    ...
    { // an optimization: compute in smaller blocks.
        ...
        int size_perblock = N * 2 * sizeof(Real);
...
template <typename Real>
void ComplexFft(std::vector<Real>* v, bool forward, std::vector<Real>* tmp_in = nullptr)
{
    assert(v != NULL);
    ...
    std::vector<Real> tmp; // allocated in ComplexFftRecursive.
...
template void RealFft(std::vector<float>* v, bool forward);
template void RealFft(std::vector<double>* v, bool forward);
}
